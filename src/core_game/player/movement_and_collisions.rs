	...
	for (entity, mut transform, collider, velocity, state) in queryyy.iter_mut() {
			...
			if let Some((_collision, toi)) = hit {
				if let TOIStatus::Converged = toi.status {
					...
					transform.translation =
						(transform.translation + original_ray * toi.toi) - direction_offset;
					collided = true;
				...
				|| state.new.0 == PlayerMoveState::Whirlwind)
		{
			let hit1 = rapier_context.cast_shape(
				...
				0.0,
				Vec2::new(
					((transform.translation.x * 8.0).round() / 8.0) - transform.translation.x,
					0.0,
				) * 1.5,
				collider,
				1.0,
			...
			let hit2 = rapier_context.cast_shape(
				...
				Vec2::new(
					0.0,
					((transform.translation.y * 8.0).round() / 8.0) - transform.translation.y,
				) * 1.5,
				collider,
				1.0,
				QueryFilter::default()
		...
		if collided == false {
			let hit3 = rapier_context.cast_shape(
				transform.translation.truncate(),
				0.0,
				(((transform.translation.truncate() * 8.0).round() / 8.0)
					- transform.translation.truncate())
					* 3.0,
				collider,
				1.0,
