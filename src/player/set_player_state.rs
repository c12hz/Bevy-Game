...
pub fn set_player_state (
    ...
    for (collider, transform, damage, mut state, mut var, mut stealth, input) in query.iter_mut() {
        ...
        let stealth_mode = input.just_pressed_skill1;
        let whirl = input.pressing_skill2;
        ...
        // WHIRLWIND MOVE STATE

        if whirl && !jump_start && var.dash_counter == 0 && var.dash_strike_counter == 0 {
            let hitx = rapier_context.cast_shape(
                Vec2::new(transform.translation.x, transform.translation.y),
                0.0,
                Vec2::new(0.0, -raycast),
                collider,
        ...
        if dash == true && var.dash_counter == 0 && var.dash_cooldown == 0 && var.dash_strike_counter == 0 {
            var.dash_counter = 5;
        }

        if var.dash_counter > 0 {
            state.old.0 = state.new.0;
            state.new.0 = PlayerMoveState::Dash;
            var.dash_counter -= 1;
            var.dash_cooldown = 15;
        }

        if var.dash_cooldown > 0 {
            var.dash_cooldown -= 1;
        }

        // DASH STRIKE STATE

        let hit_dash = rapier_context.cast_shape(
            Vec2::new(transform.translation.x, transform.translation.y),
            0.0,
            Vec2::new(0.0, -raycast),
            collider,
        ...
        if hit_dash.is_none() {
            if dash_strike == true && var.dash_strike_counter == 0 && var.dash_strike_cooldown == 0 && var.dash_counter == 0 {
            var.dash_strike_counter = 20;
            }
        }
        else {
        ...
        if var.dash_strike_counter > 0 {
            state.old.0 = state.new.0;
            state.new.0 = PlayerMoveState::DashStrike;
            var.dash_strike_counter -= 1;
            var.dash_strike_cooldown = 25;
        }

        if var.dash_strike_cooldown > 0 {
            var.dash_strike_cooldown -= 1;
        }
